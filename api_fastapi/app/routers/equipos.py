...
async def list_equipos(espacio_id: int = None, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
...
async def get_equipo(id: int, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
